...
class BookOrder(models.Model):
    ...
    please_tell_us_about_your_place     = models.CharField(max_length=100, blank=True)
    ...
    most_of_our_customers_who_book_after_builders_cleaning_also_include = models.CharField(max_length=100, blank=True)
...
class Time_id(models.Model):
    day_time = models.ForeignKey(Post, on_delete=models.CASCADE)
    time     = models.CharField( max_length=8,blank=True, null=True)
    Cost     = models.CharField( max_length=8,blank=True, null=True)
    order    = models.BooleanField(default=False)
    def __str__(self):
        return self.time

class Order_Ac(models.Model):
    order_time  = models.ForeignKey(Time_id, on_delete=CASCADE)
    accept      = models.BooleanField(default=False)
    confirm     = models.CharField(max_length=10, blank=True, null=True)
    def __str__(self):
